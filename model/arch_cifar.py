...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(F.leaky_relu(self.bn2d4(x)), scale_factor=2)#, mode = 'bilinear')
        
        x = self.deconv2(x)
       
        x = F.interpolate(F.leaky_relu(self.bn2d5(x)), scale_factor=2)#, mode = 'bilinear')
      
        x = self.deconv3(x)
        
        x = F.interpolate(F.leaky_relu(self.bn2d6(x)), scale_factor=2)#, mode = 'bilinear')
